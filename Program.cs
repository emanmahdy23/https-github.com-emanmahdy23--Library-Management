...
namespace project1
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var book2 = context.Books.FirstOrDefault();
            var authorName = book?.Author?.Name;
            var books2 = context.Books.AsNoTracking().ToList();

            var book3 = new Book { Id = 2, Title = "Updated Title" };
            ...
            int authorId = 2;
            var books3 = context.Books.FromSqlRaw("EXEC GetBookByAuthorId @p0", authorId).ToList();
            var books4 = context.Books.FromSqlInterpolated($"EXEC GetBookByAuthorId {authorId}").ToList();
